...
export default class ParticipantsList extends React.Component {
  ...
  renderParticipants() {
    ...
    return (
      <div>
        <span className="dateStrat">{dateStart}</span>
        <span className="dateEnd">{dateEnd}</span>
        <span className="timeStart">{timeStart}</span>
        <span className="timeEnd">{timeEnd}</span>
      </div>
    );
  ...
  onSaveClick(event) {
    ...
    const newPerson = {
      dateStart: this.refs.editDateStartInput.value,
      dateEnd: this.refs.editDateEndInput.value,
      timeStart: this.refs.editTimeStartInput.value,
      timeEnd: this.refs.editTimeEndInput.value
    };
